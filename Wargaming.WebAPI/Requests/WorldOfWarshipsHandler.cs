...
namespace Wargaming.WebAPI.Requests
{
	...
	{
		public WorldOfWarshipsHandler(IHttpClientFactory factory, WorldOfWarshipsHandlerOptions options) : base(factory, GetApiHost(Game.WOWS, options.Region), options.AppId, true) { }
		...
		{
			using HttpResponseMessage response = await GetRequestHandler("account/list/", new ApiArgument("search", search));
		...
		{
			using HttpResponseMessage response = await GetRequestHandler("account/info/", new ApiArgument("account_id", string.Join(',', accountIds)));
